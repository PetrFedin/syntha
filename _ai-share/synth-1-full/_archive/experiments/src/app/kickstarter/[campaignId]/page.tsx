          ...
          <Tabs defaultValue="story" className="w-full">
                  ...
                  return (
                    <Card key={update.id} className="border-primary/20 bg-primary/5">
                      <CardContent className="p-4 text-center">
                        <Lock className="mx-auto mb-4 h-8 w-8 text-primary/80" />
                        <h4 className="font-semibold">Эксклюзивное обновление</h4>
                        <p className="text-sm text-muted-foreground">
                          Это обновление доступно только для спонсоров проекта.
                        </p>
                      </CardContent>
                    </Card>
                  );
            ...
            <TabsContent value="faq" className="mt-6">
              <Card>
                <CardHeader>
                  <CardTitle className="flex items-center gap-2">
                    <HelpCircle /> Часто задаваемые вопросы
                  </CardTitle>
                </CardHeader>
            ...
            <TabsContent value="comments" className="mt-6 space-y-6">
              ...
              <form onSubmit={handleCommentSubmit} className="flex items-start gap-3 border-t pt-6">
                <Avatar>
                  <AvatarFallback>{user?.displayName?.charAt(0) || 'В'}</AvatarFallback>
                </Avatar>
                <Textarea
                  value={newComment}
            ...
            <Card>
              ...
              <CardContent className="space-y-4">
                {project.stretchGoals?.map((goal) => {
                  ...
                  const goalProgress = isAchieved
                    ? 100
                    : Math.min(100, (project.currentQuantity / goal.target) * 100);
                  return (
                    <div
                      key={goal.id}
                      className={cn('transition-opacity', !isAchieved && 'opacity-60')}
                    >
