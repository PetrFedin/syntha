import { test, expect } from '@playwright/test';
import { gotoRoleCoreCabinet } from './helpers/core-chain-overview';

const GOTO = { waitUntil: 'domcontentloaded' as const, timeout: 90_000 };

/**
 * Wave TY: brand dev tasks kanban, release gate 409, investor-readiness strip.
 * PLAYWRIGHT_SKIP_WEBSERVER=1 npm run test:e2e:core -- e2e/core-126-wave-ty-brand-dev.spec.ts
 */
test.describe('core-126: wave TY brand dev release + tasks', () => {
  test('brand dev cabinet: investor-readiness + tasks kanban mini-panel', async ({
    page,
    request,
  }) => {
    const healthRes = await request.get('/api/workshop2/platform-core/health');
    const health = (await healthRes.json()) as { demoSeeded?: boolean };
    test.skip(!health.demoSeeded, 'нужен db:core:bootstrap');

    const res = await gotoRoleCoreCabinet(page, '/brand/core?pillar=development&collection=SS27');
    expect(res?.status() ?? 599).toBeLessThan(500);
    await expect(page.getByTestId('brand-dev-cabinet-panel')).toBeVisible({ timeout: 60_000 });
    await expect(page.getByTestId('brand-dev-investor-readiness-strip')).toBeVisible({
      timeout: 45_000,
    });
    await expect(page.getByTestId('brand-dev-tasks-kanban-panel')).toBeVisible({ timeout: 45_000 });
    await expect(page.getByTestId('brand-dev-tasks-kanban-board')).toBeVisible();
  });

  test('brand calendar comms: tasks kanban mini-panel', async ({ page, request }) => {
    const healthRes = await request.get('/api/workshop2/platform-core/health');
    const health = (await healthRes.json()) as { demoSeeded?: boolean };
    test.skip(!health.demoSeeded, 'нужен db:core:bootstrap');

    const res = await page.goto('/brand/calendar?collection=SS27', GOTO);
    expect(res?.status() ?? 599).toBeLessThan(500);
    await expect(page.getByTestId('brand-dev-tasks-kanban-panel')).toBeVisible({ timeout: 60_000 });
    await expect(page.getByTestId('brand-dev-tasks-kanban-column-todo')).toBeVisible();
  });

  test('brand tasks API GET/PUT round-trip', async ({ request }) => {
    const healthRes = await request.get('/api/workshop2/platform-core/health');
    const health = (await healthRes.json()) as { pgReachable?: boolean };
    test.skip(!health.pgReachable, 'нужен PG');

    const getRes = await request.get('/api/brand/tasks');
    expect(getRes.ok()).toBeTruthy();
    const listed = (await getRes.json()) as { ok?: boolean; tasks?: unknown[] };
    expect(listed.ok).toBe(true);
    expect(Array.isArray(listed.tasks)).toBe(true);

    const taskId = `ty-e2e-${Date.now()}`;
    const putRes = await request.put('/api/brand/tasks', {
      data: {
        tasks: [
          {
            id: taskId,
            title: 'Wave TY e2e',
            status: 'todo',
            assignee: 'QA',
            due: '—',
            project: 'Разработка',
            collectionId: 'SS27',
            createdAt: new Date().toISOString(),
            updatedAt: new Date().toISOString(),
          },
        ],
      },
    });
    expect(putRes.ok()).toBeTruthy();
  });

  test('investor-readiness API for dev strip', async ({ request }) => {
    const res = await request.get('/api/workshop2/investor-readiness');
    expect(res.ok()).toBeTruthy();
    const json = (await res.json()) as {
      readyForInvestorDemo?: boolean;
      ss27?: { articleCount?: number; avgTzFillPct?: number | null };
    };
    expect(typeof json.readyForInvestorDemo).toBe('boolean');
    expect(json.ss27?.articleCount).toBeGreaterThanOrEqual(0);
  });

  test('release gate POST returns 409 when passport incomplete (fail-closed)', async ({
    request,
  }) => {
    const res = await request.post('/api/brand/merch/release-gate', {
      data: { collectionId: 'SS27' },
    });
    const json = (await res.json()) as { messageRu?: string; summary?: { total: number; ready: number } };
    expect(json.messageRu?.length).toBeGreaterThan(0);
    if (res.status() === 409) {
      expect(json.messageRu).toContain('material passport');
    } else {
      expect(res.ok()).toBeTruthy();
    }
  });

  test('brand OP dossier: attach TZ→PO peer link', async ({ page, request }) => {
    const healthRes = await request.get('/api/workshop2/platform-core/health');
    const health = (await healthRes.json()) as { demoSeeded?: boolean };
    test.skip(!health.demoSeeded, 'нужен db:core:bootstrap');

    const res = await page.goto('/brand/b2b-orders/B2B-SS27-DEMO-001?collection=SS27', GOTO);
    expect(res?.status() ?? 599).toBeLessThan(500);
    await expect(page.getByTestId('brand-op-attach-tz-po-strip')).toBeVisible({ timeout: 60_000 });
    await expect(page.getByTestId('brand-op-attach-tz-po-link')).toBeVisible();
  });
});
