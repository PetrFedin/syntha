...
/**
 * Wave YQ: shop CO checkout 2.2 — env-gated payment intent (honest RU stub badge when keys absent),
 * checkout → tracking after create, deduped payment CTAs.
 * PLAYWRIGHT_SKIP_WEBSERVER=1 npm run test:e2e:core -- e2e/core-232-wave-yq-checkout.spec.ts
 */
test.describe('core-232: wave YQ shop checkout payment polish', () => {
  ...
  test('checkout: honest stub-absent badge + deduped CTA + tracking cross-link', async ({
    page,
  }) => {
    ...
    const stubAbsent = page.getByTestId('shop-co-checkout-payment-intent-badge-stub-absent');
    ...
    if (await notConnected.isVisible().catch(() => false)) {
      await expect(stubAbsent).toBeVisible();
      await expect(stubAbsent).toContainText(/Заглушка/i);
    } else if (
  ...
  test('POST payment-intent with amount returns structured stub/live result', async ({
    ...
    const res = await request.post('/api/shop/b2b/checkout/payment-intent', {
      data: {
        amountRub: 15000,
        orderId: 'B2B-YQ-E2E-001',
        returnUrl: 'http://localhost:3001/shop/b2b/orders/tracking?order=B2B-YQ-E2E-001',
      },
    });
  ...
  test('checkout golden path — tracking link, no duplicate payment nav', async ({ page }) => {
