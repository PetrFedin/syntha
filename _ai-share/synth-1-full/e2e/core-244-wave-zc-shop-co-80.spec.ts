...
import { WAVE_ZC_SHOP_CO_CELL_SCORE_MIN } from '../src/lib/platform/wave-zc-shop-co-80-bump';
...
/**
 * Wave ZC: shop CO audit §6 8.0 — golden path + chain + cross-links (YK/XT/WM/XL).
 * PLAYWRIGHT_SKIP_WEBSERVER=1 npm run test:e2e:core -- e2e/core-244-wave-zc-shop-co-80.spec.ts
 */
test.describe('core-244: wave ZC shop CO audit 8.0', () => {
  test('matrix — golden path RU + spine peers', async ({ page, request }) => {
    const health = (await (
  ...
  test('tracking — chain peer mirror visible', async ({ page }) => {
    ...
    await expect(page.getByTestId('shop-co-chain-peer-po-synced').or(
      page.getByTestId('shop-co-chain-peer-po-pending')
  ...
  test('hub audit — shop CO cell score >= 8.0', async ({ page, request }) => {
    ...
    expect(shopCo?.staticScore).toBeGreaterThanOrEqual(WAVE_ZC_SHOP_CO_CELL_SCORE_MIN);
