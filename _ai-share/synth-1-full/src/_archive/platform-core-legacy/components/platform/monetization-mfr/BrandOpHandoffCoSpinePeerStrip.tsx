...
export function BrandOpHandoffCoSpinePeerStrip({ orderId, collectionId, factoryId }: Props) {
  ...
  const matrixHref = platformCoreUiHref(
    `${ROUTES.shop.b2bMatrix}?collection=${encodeURIComponent(collectionId)}&order=${encodeURIComponent(orderId)}`
  );
  const checkoutHref = platformCoreUiHref(
    `${ROUTES.shop.b2bCheckout}?collection=${encodeURIComponent(collectionId)}&order=${encodeURIComponent(orderId)}`
  );
  ...
  return (
    ...
    >
      <Link
        href={matrixHref}
        data-testid="brand-op-handoff-shop-matrix-link"
      ...
      <Link
        href={checkoutHref}
        data-testid="brand-op-handoff-shop-checkout-link"
        className={hubGadget.goldenLink}
      >
        Оформление
      </Link>
      ...
      <Link
        ...
        data-testid="brand-op-handoff-shop-tracking-link"
        className={hubGadget.goldenLink}
      >
        Трекинг магазина
      </Link>
      ...
      <Link
        ...
        data-testid="brand-op-handoff-mfr-comms-link"
        className={hubGadget.goldenLink}
      >
        Связь с цехом
      </Link>
      ...
      <Link
        href={session.shopReplenishmentAtpHref}
