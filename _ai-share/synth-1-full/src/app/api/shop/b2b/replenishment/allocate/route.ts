...
import { allocateShopReplenishmentFromWmsAtp } from '@/lib/server/shop-replenishment-allocate-server';
import { guardShopB2bCheckoutRoute } from '@/lib/server/shop-b2b-checkout-route-auth';
import { resolveShopCoreBuyerIdFromRequest } from '@/lib/order/shop-core-buyer-context';

/** POST — WMS ATP → matrix lines + prefill deep-link (Wave TV). */
export async function POST(req: NextRequest) {
  ...
  const result = await allocateShopReplenishmentFromWmsAtp({
    buyerId,
    collectionId,
    orderId,
    sessionId,
