/**
 * POST/GET — comment-only annotations к read-only ТЗ (Wave XN · factory path).
 */
import { NextRequest, NextResponse } from 'next/server';

import { jsonWorkshop2ErrorRu } from '@/lib/production/workshop2-api-error-ru';
import { withWorkshop2ApiErrorRu } from '@/lib/production/workshop2-api-route-ru';
import { FACTORY_DOSSIER_COMMENTS_API_PATH } from '@/lib/production/mfr-dossier-comments-wave-xn';
...
export const POST = withWorkshop2ApiErrorRu(async function postFactoryDossierComment(
  ...
  const actor =
    resolveWorkshop2UpdatedBy(req, String(b.actor ?? ''), auth.actor) ?? 'factory-dossier-comment';
  ...
  return NextResponse.json({
    ...
    apiPath: FACTORY_DOSSIER_COMMENTS_API_PATH,
  });
});
