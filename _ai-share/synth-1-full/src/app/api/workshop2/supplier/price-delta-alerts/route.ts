...
export const GET = withWorkshop2ApiErrorRu(async function getSupplierPriceDeltaAlerts(req: NextRequest) {
  const auth = await guardWorkshop2Route(req, WORKSHOP2_EVENTS_READ_ROLES);
  if (auth instanceof NextResponse) return auth;

  const { searchParams } = new URL(req.url);
  const collectionId = String(searchParams.get('collectionId') ?? searchParams.get('collection') ?? '').trim();
  ...
  const thresholdPct = thresholdRaw ? Number(thresholdRaw) : SUPPLIER_PRICE_DELTA_DEFAULT_THRESHOLD_PCT;
