...
/** Push materials_supplied на supplier comms cabinet — PG events + chain-status SSE bump. */
export function SupplierCommsChainMaterialsPushStrip({
  orderId,
  materialsDone,
    ...
    void (async () => {
      try {
        const res = await fetch(
          `/api/platform-core/notification-events?role=supplier&orderId=${encodeURIComponent(oid)}&limit=6`,
        ...
        const materials = (json.events ?? []).find(
          (row) => row.kind === WAVE_YO_SUP_CM_CHAIN_MATERIALS_PUSH_STEP
        );
        if (materials) {
          setEvent({
            id: materials.id,
  ...
  const testId = supCmChainMaterialsPushTestId(orderId);
  ...
  return (
    <div
      className={cn(
        'flex flex-wrap items-center gap-2 rounded-md border px-2 py-1.5 text-[10px]',
        materialsDone
          ? 'border-emerald-200 bg-emerald-50/80 text-emerald-900'
          : 'border-amber-200 bg-amber-50/80 text-amber-900'
      )}
      data-testid={testId}
      {...{ [WAVE_YO_SUP_CM_CHAIN_MATERIALS_SSE_ATTR]: sseLive ? '1' : '0' }}
      {...{ [WAVE_YO_SUP_CM_CHAIN_MATERIALS_PUSH_ATTR]: showPushBadge ? '1' : '0' }}
    >
      <Badge variant="outline" className="border-current/30 text-[9px]">
        {supCmChainMaterialsPushBadgeRu(materialsDone)}
      </Badge>
      {showPushBadge ? (
        <Badge
          variant="outline"
          className="border-emerald-400 bg-emerald-100/80 text-[8px] uppercase tracking-wide text-emerald-900"
          data-testid={supCmChainMaterialsSuppliedPushBadgeTestId(orderId)}
        >
          {WAVE_YO_SUP_CM_CHAIN_MATERIALS_PUSH_STEP}
        </Badge>
      ) : null}
      <span className="min-w-0 flex-1 font-medium">
        {event?.titleRu ??
          (materialsDone
            ? 'Материалы подтверждены · chain-status'
            : supCmChainMaterialsPushFallbackRu())}
      </span>
      {event?.bodyRu ? (
      ...
      <Link
        href={factorySupplierCalendarB2bOrderContextHref(orderId)}
