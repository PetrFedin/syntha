...
export function BrandDevCabinetCoPeerStrip({ collectionId, orderId }: Props) {
  ...
  return (
    <div className={hubGadget.goldenPath} data-testid={WAVE_WZ_BRAND_DEV_CO_PEER_STRIP_TESTID}>
      <Link href={linesheetsHref} data-testid="brand-dev-cabinet-linesheets-link" className={hubGadget.goldenLink}>
        {WAVE_WZ_BRAND_DEV_CO_LINESHEETS_RU}
      </Link>
      <span className={hubGadget.goldenSep} aria-hidden>
        ·
      </span>
      <Link href={shop.showroomHref} data-testid="brand-dev-cabinet-shop-showroom-link" className={hubGadget.goldenLink}>
        {WAVE_WZ_BRAND_DEV_CO_SHOWROOM_RU}
      </Link>
      <span className={hubGadget.goldenSep} aria-hidden>
        ·
      </span>
      <Link href={crmHref} data-testid="brand-dev-cabinet-crm-segments-link" className={hubGadget.goldenLink}>
        Сегменты CRM
      </Link>
    </div>
  );
