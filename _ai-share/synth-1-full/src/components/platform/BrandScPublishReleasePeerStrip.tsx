...
export function BrandScPublishReleasePeerStrip({ collectionId }: Props) {
  ...
  const showroomPublishHref = platformCoreUiHref(
    `${ROUTES.brand.launchReadiness}?${PILLAR_CAPABILITY_FEATURE_PARAM}=showroom-publish&collection=${encodeURIComponent(collectionId)}`
  );
  ...
  return (
    ...
    >
      <Link
        ...
        data-testid="brand-sc-publish-release-gate-link"
      ...
      <Link
        ...
        data-testid="brand-sc-publish-showroom-tab-link"
        className={hubGadget.goldenLink}
      >
        Публикация шоурума
      </Link>
      ...
      <Link
        ...
        data-testid="brand-sc-publish-syndication-link"
        className={hubGadget.goldenLink}
      >
        Синдикация
      </Link>
      ...
      <Link
        ...
        data-testid="brand-sc-publish-platform-hub-link"
        className={hubGadget.goldenLink}
      >
        Платформа B2B
      </Link>
      ...
      <Link
        href={matrixHref}
        data-testid="brand-sc-publish-shop-matrix-link"
        className={hubGadget.goldenLink}
      >
        Матрица магазина
      </Link>
      ...
      <Link
        href={checkoutHref}
        data-testid="brand-sc-publish-shop-checkout-link"
        className={hubGadget.goldenLink}
      >
        Оформление
      </Link>
