...
import { isPlatformCoreEmptyChainCollection, getPlatformCoreDemo } from '@/lib/platform-core-demo-context';
...
/** Baseline brand/shop comms pillar — без factory/extended imports. */
export function CommsPillarCardBaseline({ variant, compact = false, minimalChrome = false }: Props) {
  ...
  const { demoOrderId: fallbackOrderId, collectionId, demoArticleId } = demo;
  ...
  const pgCabinet: PgContextualThreadsCabinet = variant;
  const {
  ...
  const messagesHref =
    variant === 'shop'
      ? shopMessagesB2bOrderContextHref(cabinetOrderId)
      : brandMessagesB2bOrderContextHref(cabinetOrderId);
  const calendarHref =
    variant === 'shop'
      ? shopCalendarB2bOrderContextHref(cabinetOrderId)
      : brandCalendarB2bOrderContextHref(cabinetOrderId);
  const inboxAllHref = variant === 'shop' ? ROUTES.shop.messages : ROUTES.brand.messages;
  const articleChatHref =
    variant === 'shop'
      ? shopMessagesWorkshop2ArticleContextHref(collectionId, demoArticleId)
      : brandMessagesWorkshop2ArticleContextHref(collectionId, demoArticleId);
  ...
  const orderChatTestId =
    variant === 'brand' ? 'brand-cm-order-chat-link' : 'shop-cm-order-chat-link';
  const articleChatTestId =
    variant === 'brand' ? 'brand-cm-article-chat-link' : 'shop-cm-article-chat-link';
  const calendarTestId =
    variant === 'brand' ? 'brand-cm-calendar-link' : 'shop-cm-calendar-link';

  const notesTestId = variant === 'brand' ? 'brand-cm-notes-link' : 'shop-cm-notes-link';
      ...
      <CardContent className={cn(compact ? hubGadget.pillarBody : 'space-y-2 p-3 text-xs')}>
        ...
        {compact && !emptyChain && cabinetOrderId && !minimalChrome && !suppressChainBadge ? (
          <PlatformCoreChainStatusRefreshBadge
            ...
            pollTestId={
              variant === 'brand' ? 'brand-cm-cabinet-poll-badge' : 'shop-cm-cabinet-poll-badge'
            }
          />
        ) : null}
        {statusLine ? (
        ...
        {!emptyChain && !minimalChrome && (!compact || auditUi) ? (
