...
interface SheetContentProps
  ...
  /** Fallback для a11y, если в children нет SheetTitle */
...
function containsDialogTitle(node: React.ReactNode): boolean {
  ...
  if (Array.isArray(node)) {
    return node.some((child) => containsDialogTitle(child as React.ReactNode));
  }
  if (!React.isValidElement(node)) return false;
  if (node.type === SheetPrimitive.Title) return true;
  const childProps = node.props as { children?: React.ReactNode };
  if (childProps.children != null) return containsDialogTitle(childProps.children);
  return false;
}
...
const SheetContent = React.forwardRef<
  ...
  SheetContentProps
>(({ side = 'right', className, children, accessibilityTitle = 'Меню', ...props }, ref) => {
