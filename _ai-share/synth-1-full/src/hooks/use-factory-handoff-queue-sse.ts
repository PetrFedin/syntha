...
export function useFactoryHandoffQueueSse(
  ...
  useEffect(() => {
    ...
    const url = `/api/workshop2/factory/production-handoff-queue/stream?factoryId=${encodeURIComponent(factoryId)}`;
    const es = new EventSource(url);
    es.onopen = () => setSseConnected(true);
    es.onmessage = (ev) => {
      try {
        const data = JSON.parse(ev.data) as { type?: string };
        if (data.type === 'handoff_queue_update') refresh();
      } catch {
        /* ignore */
      }
    };
