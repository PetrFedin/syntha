...
import {
  WAVE_YU_BRAND_SC_MATRIX_DEDUP_FIXES,
  WAVE_YU_BRAND_SC_MINI_MATRIX_LINK_TESTID,
  WAVE_YU_BRAND_SC_MINI_MATRIX_STRIP_TESTID,
  WAVE_YU_BRAND_SC_OPEN_SHOP_BTN_TESTID,
  brandScCrossMatrixOneClickAriaLabelRu,
} from '@/lib/platform/wave-yu-brand-sc-mini-matrix-cta';
...
describe('wave YU — brand SC mini-matrix one-click CTA', () => {
  it('one-click aria labels for mini-matrix and open-shop variants', () => {
  ...
  it('wave YU testids are stable', () => {
    expect(WAVE_YU_BRAND_SC_MINI_MATRIX_STRIP_TESTID).toBe('brand-sample-collection-mini-matrix');
    expect(WAVE_YU_BRAND_SC_MINI_MATRIX_LINK_TESTID).toBe('brand-sc-mini-matrix-link');
    expect(WAVE_YU_BRAND_SC_OPEN_SHOP_BTN_TESTID).toBe('brand-sc-cross-matrix-open-shop-btn');
  });

  it('documents matrix CTA dedup surfaces', () => {
    expect(WAVE_YU_BRAND_SC_MATRIX_DEDUP_FIXES.length).toBe(5);
    expect(WAVE_YU_BRAND_SC_MATRIX_DEDUP_FIXES.map((f) => f.surface)).toEqual([
      'cabinet',
      'cabinet',
      'linesheets',
      'showroom',
  ...
  it.each(WAVE_YU_BRAND_SC_MATRIX_DEDUP_FIXES)('$id — dedup wired in source', (fix) => {
