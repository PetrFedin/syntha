...
describe('wave YX — notification center compact all roles final (S4)', () => {
  ...
  it('prefs dedup — only inside CommsNotificationCenterStrip (wave VK/WZ)', () => {
    ...
    for (const wire of WAVE_YX_PILLAR_NOTIFICATION_COMPACT_WIRES) {
  ...
  it('core-248 e2e spec — file on disk + playwright.core.config.ts entry', () => {
    const pkgRoot = path.join(SRC, '..');
    expect(fs.existsSync(path.join(pkgRoot, 'e2e', WAVE_YX_E2E_SPEC))).toBe(true);
    const config = fs.readFileSync(path.join(pkgRoot, 'playwright.core.config.ts'), 'utf8');
    expect(config).toContain(`**/${WAVE_YX_E2E_SPEC}`);
  });
});
