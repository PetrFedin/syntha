...
describe('workshop2-pg-read-path-policy', () => {
  ...
  it('golden SS27 in core mode: api when PG live, localStorage when PG down', () => {
    ...
    expect(resolveWorkshop2HubPublishedArticlesReadPath({ collectionId: 'SS27', preferApi: true })).toBe(
      'api'
    );
    expect(resolveWorkshop2HubPublishedArticlesReadPath({ collectionId: 'SS27', preferApi: false })).toBe(
      'localStorage'
    );
  ...
  it('non-golden EMPTY27 in core mode: localStorage when PG down', () => {
    process.env.NEXT_PUBLIC_PLATFORM_CORE_MODE = '1';
    expect(resolveWorkshop2HubPublishedArticlesReadPath({ collectionId: 'EMPTY27', preferApi: false })).toBe(
      'localStorage'
    );
  });
