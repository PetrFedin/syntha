  ...
  return Object.freeze({
    ...
    async snapshot() {
      const [organisations, memberships, relationships, invitations, campaigns, collections, showrooms, selections, orders, cycles, deals, calendar, commands, outbox] = await Promise.all([
        payloads(pool, 'organisations'), payloads(pool, 'memberships'), payloads(pool, 'counterparty_relationships'),
        payloads(pool, 'showroom_invitations'), payloads(pool, 'campaigns'), payloads(pool, 'collections'), payloads(pool, 'showrooms'),
        payloads(pool, 'selections'), payloads(pool, 'orders'), payloads(pool, 'commercial_cycles'), payloads(pool, 'deals'),
        payloads(pool, 'calendar_milestones'), commandPayloads(pool, 'commands'), outboxRecords(pool),
      ]);
      return Object.freeze({
        organisations, memberships, relationships, showroomInvitations: invitations, campaigns, collections, showrooms,
        selections, orders, cycles, deals, calendar, commands, outbox, events: outbox.map((record) => record.event),
      });
    },
    readOutbox(status = 'pending') { return readOutbox(pool, status); },
...
function transactionView(client) {
  return Object.freeze({
    ...
    insertMembership: (value) => insert(client, 'memberships', ['id', 'organisation_id', 'user_id', 'organisation_type', 'role', 'status', 'payload'], [value.id, value.organisationId, value.userId, value.organisationType, value.role, value.status, value], 'MEMBERSHIP_ALREADY_EXISTS'),
    ...
    insertRelationship: (value) => insert(client, 'counterparty_relationships', ['id', 'brand_id', 'shop_id', 'status', 'version', 'payload'], [value.id, value.brandId, value.shopId, value.status, value.version, value], 'RELATIONSHIP_ALREADY_EXISTS'),
    saveRelationship: (value, expectedVersion) => saveVersioned(client, 'counterparty_relationships', value, expectedVersion, ['status'], [value.status], 'RELATIONSHIP_CONCURRENCY_CONFLICT'),
    ...
    insertShowroomInvitation: (value) => insert(client, 'showroom_invitations', ['id', 'showroom_id', 'relationship_id', 'brand_id', 'shop_id', 'status', 'expires_at', 'version', 'payload'], [value.id, value.showroomId, value.relationshipId, value.brandId, value.shopId, value.status, value.expiresAt, value.version, value], 'SHOWROOM_INVITATION_ALREADY_EXISTS'),
    saveShowroomInvitation: (value, expectedVersion) => saveVersioned(client, 'showroom_invitations', value, expectedVersion, ['status', 'expires_at', 'relationship_id'], [value.status, value.expiresAt, value.relationshipId], 'SHOWROOM_INVITATION_CONCURRENCY_CONFLICT'),
    ...
    insertCollection: (value) => insert(client, 'collections', ['id', 'campaign_id', 'brand_id', 'status', 'currency', 'version', 'payload'], [value.id, value.campaignId, value.brandId, value.status, value.currency, value.version, value], 'COLLECTION_ALREADY_EXISTS'),
    ...
    insertSelection: (value) => insert(client, 'selections', ['id', 'cycle_id', 'showroom_id', 'collection_id', 'brand_id', 'shop_id', 'status', 'version', 'payload'], [value.id, value.cycleId, value.showroomId, value.collectionId, value.brandId, value.shopId, value.status, value.version, value], 'SELECTION_ALREADY_EXISTS'),
    ...
    insertOrder: (value) => insert(client, 'orders', ['id', 'selection_id', 'cycle_id', 'brand_id', 'shop_id', 'status', 'currency', 'total_amount', 'version', 'payload'], [value.id, value.selectionId, value.cycleId, value.brandId, value.shopId, value.status, value.currency, value.totalAmount, value.version, value], 'ORDER_ALREADY_EXISTS'),
    saveOrder: (value, expectedVersion) => saveVersioned(client, 'orders', value, expectedVersion, ['status', 'currency', 'total_amount'], [value.status, value.currency, value.totalAmount], 'ORDER_CONCURRENCY_CONFLICT'),

    getCycle: (id) => getPayload(client, 'commercial_cycles', 'id', id),
    insertCycle: (value) => insert(client, 'commercial_cycles', ['id', 'brand_id', 'shop_id', 'campaign_id', 'collection_id', 'stage', 'version', 'payload'], [value.id, value.brandId, value.shopId, value.campaignId, value.collectionId, value.stage, value.version, value], 'CYCLE_ALREADY_EXISTS'),
    ...
    insertDeal: (value) => insert(client, 'deals', ['id', 'cycle_id', 'order_id', 'brand_id', 'shop_id', 'status', 'payload'], [value.id, value.cycleId, value.orderId, value.brandId, value.shopId, value.status, value], 'DEAL_ALREADY_EXISTS'),
    insertCalendarMilestone: (value) => insert(client, 'calendar_milestones', ['id', 'owner_organisation_id', 'cycle_id', 'type', 'starts_at', 'visibility', 'payload'], [value.id, value.ownerOrganisationId, value.cycleId, value.type, value.startsAt, value.visibility, value], 'CALENDAR_MILESTONE_ALREADY_EXISTS'),
    ...
    insertCommand: (value) => insertCommand(client, 'commands', value),
    appendOutbox: (event) => insert(client, 'outbox_events', ['id', 'event_type', 'aggregate_id', 'status', 'event', 'published_at'], [event.id, event.type, event.aggregateId, 'pending', event, null], 'OUTBOX_EVENT_ALREADY_EXISTS'),
...
async function saveVersioned(client, table, value, expectedVersion, columns, values, code) {
  invariant(value.version === expectedVersion + 1, 'VERSION_INCREMENT_INVALID', 'Version must increment exactly once', { id: value.id, expectedVersion, nextVersion: value.version });
...
function commandFromRow(row) {
  ...
  return Object.freeze({ id: row.id, fingerprint: row.fingerprint, actorId: row.actor_id, result: row.result, completedAt: row.completed_at.toISOString?.() ?? row.completed_at });
}

async function insertCommand(client, table, value) {
  try {
    await client.query(`INSERT INTO ${table} (id, fingerprint, actor_id, result, completed_at) VALUES ($1, $2, $3, $4::jsonb, $5)`, [value.id, value.fingerprint, value.actorId, JSON.stringify(value.result), value.completedAt]);
  } catch (error) {
...
async function readOutbox(queryable, status) {
  const result = await queryable.query('SELECT event, status, published_at FROM outbox_events WHERE status = $1 ORDER BY id', [status]);
  return result.rows.map((row) => Object.freeze({ event: row.event, status: row.status, publishedAt: row.published_at?.toISOString?.() ?? row.published_at }));
}
