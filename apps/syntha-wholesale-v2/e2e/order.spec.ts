...
async function createReadySelection(request: APIRequestContext, suffix: string) {
  ...
  const readyCollection = await json<{ readonly version: number }>(
...
test('buyer submits an immutable Order and seller receives the contract', async ({
  ...
  const selection = await createReadySelection(request, suffix);
  const createHeaders = {
    ...selectionE2eBuyerHeaders,
  ...
  const createReplay = await request.post('/api/orders', {
    headers: createHeaders,
    data: { selectionId: selection.id },
  });
  expect(createReplay.status()).toBe(200);
  expect(createReplay.headers()['idempotency-replayed']).toBe('true');
  expect(await createReplay.json()).toEqual(order);

  const sellerDraftRead = await request.get(`/api/orders/${encodeURIComponent(order.id)}`, {
    headers: lifecycleE2eHeaders,
  });
  ...
  const submitReplay = await request.post(
    `/api/orders/${encodeURIComponent(order.id)}/submit`,
    {
      headers: submitHeaders,
      data: { expectedVersion: resized.version },
  ...
  expect(await submitReplay.json()).toEqual(snapshot);

  const sellerSnapshot = await json<{
    readonly id: string;
    readonly orderId: string;
    readonly totals: { readonly quantity: number; readonly totalMinor: number };
  }>(
  ...
  expect(sellerSnapshot).toMatchObject({
    id: snapshot.id,
    orderId: order.id,
    totals: { quantity: 6, totalMinor: 162_000 },
  });
  ...
  await expect(
    page.getByTestId('buyer-submitted-order-card').filter({ hasText: snapshot.id }),
  ).toBeVisible();

  await page.setExtraHTTPHeaders(lifecycleE2eHeaders);
  await page.goto('/orders');
  const sellerCard = page
    .getByTestId('seller-submitted-order-card')
    .filter({ hasText: snapshot.id });
  await expect(sellerCard).toBeVisible();
  await expect(sellerCard.getByText(/1[\s\u00a0\u202f]?620,00/).first()).toBeVisible();
});
