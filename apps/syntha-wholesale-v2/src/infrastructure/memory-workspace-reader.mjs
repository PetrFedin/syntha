...
export function createMemoryWorkspaceReader({ store, catalogStore, productSpecificationStore } = {}) {
  ...
  return Object.freeze({
    async readForActor(actorId) {
      ...
      const specificationSource = productSpecificationStore && typeof productSpecificationStore.snapshot === 'function'
        ? await productSpecificationStore.snapshot()
        : { materials: [], materialRevisions: [], boms: [] };
      ...
      const specificationBrandIds = new Set(
        memberships
          .filter((item) => item.organisationType === 'brand' && membershipHasCapability(item, CAPABILITIES.PRODUCT_SPECIFICATION_READ))
      ...
      const showrooms = array(source.showrooms).filter((item) => showroomIds.has(item.id) || ownIds.has(item.brandId));
      ...
      return {
        memberships,
        organisations: array(source.organisations).filter((item) => visibleOrgIds.has(item.id)),
        relationships,
        invitations,
        campaigns: array(source.campaigns).filter((item) => campaignIds.has(item.id) || ownIds.has(item.brandId)),
        collections: array(source.collections).filter((item) => collectionIds.has(item.id) || ownIds.has(item.brandId)),
        sizeGrids,
        styles,
        materials: array(specificationSource.materials).filter((item) => specificationBrandIds.has(item.brandId)),
        materialRevisions: array(specificationSource.materialRevisions).filter((item) => specificationBrandIds.has(item.brandId)),
        boms: array(specificationSource.boms).filter((item) => specificationBrandIds.has(item.brandId)),
        catalogSkus: array(catalogSource.skus).filter((item) => ownBrandIds.has(item.brandId) || (item.status === 'published' && collectionIds.has(item.collectionId))),
        showrooms,
        cycles,
        selections,
        orders,
        deals,
        calendar: array(source.calendar).filter((item) => ownIds.has(item.ownerOrganisationId)),
      };
    },
  });
}
