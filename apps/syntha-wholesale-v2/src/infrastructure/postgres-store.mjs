...
export function createPostgresWholesaleStore({ pool }) {
  ...
  return Object.freeze({
    transaction,
    async snapshot() {
      const [organisations, memberships, relationships, invitations, campaigns, collections, sizeGrids, styles, showrooms, selections, orders, cycles, deals, calendar, commands, outbox] = await Promise.all([
        ...
        payloads(pool, 'product_size_grids'),
        payloads(pool, 'product_styles'),
...
function transactionView(client) {
  return Object.freeze({
    ...
    getSizeGrid: (id) => getPayload(client, 'product_size_grids', 'id', id),
    getSizeGridByCode: (brandId, code) => getPayloadBy(client, 'product_size_grids', ['brand_id', 'code'], [brandId, code]),
    insertSizeGrid: (value) => insert(
      client,
      'product_size_grids',
      ['id', 'brand_id', 'code', 'status', 'version', 'payload'],
      [value.id, value.brandId, value.code, value.status, value.version, value],
      'SIZE_GRID_ALREADY_EXISTS',
    ),
    saveSizeGrid: (value, expectedVersion) => saveVersioned(
      client,
      'product_size_grids',
      value,
      expectedVersion,
      ...
      'SIZE_GRID_CONCURRENCY_CONFLICT',
    ),
    getStyle: (id) => getPayload(client, 'product_styles', 'id', id),
    getStyleByCode: (brandId, styleCode) => getPayloadBy(client, 'product_styles', ['brand_id', 'style_code'], [brandId, styleCode]),
    insertStyle: (value) => insert(
      client,
      'product_styles',
      ['id', 'brand_id', 'collection_id', 'style_code', 'size_grid_id', 'status', 'version', 'payload'],
      [value.id, value.brandId, value.collectionId, value.styleCode, value.sizeGrid.id, value.status, value.version, value],
      'STYLE_ALREADY_EXISTS',
    ),
    saveStyle: (value, expectedVersion) => saveVersioned(
      client,
      'product_styles',
      value,
      expectedVersion,
      ['style_code', 'size_grid_id', 'status'],
      [value.styleCode, value.sizeGrid.id, value.status],
      'STYLE_CONCURRENCY_CONFLICT',
    ),
