...
export function createPostgresWholesaleRuntime({
  ...
  const transport = { authenticate: auth.authenticate, auth, readiness, platform, catalog, productDevelopment, partners, collaboration, orders, notifications, workspace };
  const handler = createWholesaleHttpHandler(transport);
  const fetchHandler = createWholesaleFetchHandler(transport);
  return Object.freeze({ auth, readiness, store, catalogStore, platform, catalog, productDevelopment, partners, collaboration, orders, notifications, workspace, handler, fetchHandler });
}
