...
export function createPostgresWholesaleRuntime({ pool, migrationsDir, clock, nextId, randomBytesImpl, sessionTtlMs, maxLoginFailures, loginWindowMs, loginBlockMs, revokedSessionRetentionMs } = {}) {
  ...
  const store = createPostgresWholesaleStore({ pool }); const catalogStore = createPostgresCatalogStore({ pool }); const productSpecificationStore = createPostgresProductSpecificationStore({ pool }); const measurementStore = createPostgresMeasurementStore({ pool });
  ...
  const platform = createWholesalePlatform(options); const catalog = createCatalogService({ wholesaleStore: store, catalogStore, ...(clock ? { clock } : {}), ...(nextId ? { nextId } : {}) }); const productDevelopment = createProductDevelopmentService(options); const productSpecification = createProductSpecificationService({ store: productSpecificationStore, ...(clock ? { clock } : {}), ...(nextId ? { nextId } : {}) }); const measurements = createMeasurementService({ store: measurementStore, ...(clock ? { clock } : {}), ...(nextId ? { nextId } : {}) }); const partners = createPartnerAccessService(options); const collaboration = createShowroomSelectionService({ ...options, catalogReader: catalog }); const orders = createOrderBuilderService(options);
  ...
  const transport = { authenticate: auth.authenticate, auth, readiness, platform, catalog, productDevelopment, productSpecification, measurements, partners, collaboration, orders, notifications, workspace }; const handler = createWholesaleHttpHandler(transport); const fetchHandler = createWholesaleFetchHandler(transport);
  return Object.freeze({ auth, readiness, store, catalogStore, productSpecificationStore, measurementStore, platform, catalog, productDevelopment, productSpecification, measurements, partners, collaboration, orders, notifications, workspace, handler, fetchHandler });
}
