...
test('selection upsert is deterministic and submission requires lines', () => {
  ...
  const withLine = upsertSelectionLine(selection, { sku: 'SKU-1', quantity: 2, unitPrice: 100 }, 'buyer-1', now);
  const replaced = upsertSelectionLine(withLine, { sku: 'SKU-1', quantity: 3, unitPrice: 100 }, 'buyer-1', now);
