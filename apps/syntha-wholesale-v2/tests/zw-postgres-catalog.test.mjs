...
test('PostgreSQL catalog persists immutable published SKU snapshots', { skip: !databaseUrl }, async () => {
  ...
  try {
    ...
    const draft = await catalog.createSku('catalog-create', 'sales-pg', { sku: 'PG-SKU-1', collectionId: collection.id, brandId: 'brand-pg', name: 'Postgres Jacket', wholesalePrice: 125.5, currency: 'EUR' });
    ...
    const rows = await pool.query('SELECT status, currency, wholesale_price::text AS price, version, payload FROM catalog_skus WHERE sku = $1', [draft.sku]);
